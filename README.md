...
```hcl
github.com/pbs/terraform-aws-elasticache-redis-standalone-module?ref=3.1.4
```
...
```hcl
module "redis" {
  source = "github.com/pbs/terraform-aws-elasticache-redis-standalone-module?ref=3.1.4"
...
| <a name="input_extra_parameters"></a> [extra\_parameters](#input\_extra\_parameters) | Additional parameters that will be added to parameter group. | <pre>list(object({<br/>    name  = string<br/>    value = string<br/>  }))</pre> | `[]` | no |
| <a name="input_extra_security_group_ids"></a> [extra\_security\_group\_ids](#input\_extra\_security\_group\_ids) | Additional VPC security groups to associate with the nodes, appended to `security_group_ids` or to the one provided by this module. | `list(string)` | `[]` | no |
